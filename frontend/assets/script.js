...
document.getElementById('loginForm').addEventListener('submit', async (e) => {
  ...
  const loginSucesso = await fazerLogin(username, password);
...
// Envia os dados para o backend Node.js (não mais diretamente para o Google Apps Script)
async function submitForm(data, tipo = 'venda') {
  try {
    // Adiciona o tipo aos dados
    data.tipo = tipo;
    
    const endpoint = tipo === 'venda' ? '/venda' : '/posvenda';
...
async function buscarClientes(termo) {
  try {
    ...
    const supervisorName = user.username;
    
    console.log('Buscando com termo:', termo, 'Supervisor:', supervisorName);
    
    const response = await fetch(`${API_URL}/clientes/busca?termo=${encodeURIComponent(termo)}&supervisor=${encodeURIComponent(supervisorName)}`, {
      headers: {
        'Authorization': `Bearer ${authToken}`
      }
    });
...
async function carregarListaClientes() {
  try {
    ...
    // Buscar todos os clientes (apenas os do supervisor logado)
    const response = await fetch(`${API_URL}/clientes?supervisor=${encodeURIComponent(supervisorName)}`, {
      headers: {
        'Authorization': `Bearer ${authToken}`
      }
    });
